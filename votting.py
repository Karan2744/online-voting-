...
def resultsection(): #pollresults
    def results():
        ...
        if sel=='-select-':
            ...
            Button(res,text='Project Results',width=90,height=3,bg='lightblue',font='20',command=project).place(x=40,y=400)
...
def votingsystem():
    ...
    Button(ppage,text='Vote',font='20',width=20,height=2,bg='#6dcff6',command=proceed).place(x=550,y=230)
...
def user():
    ...
    def signup():
        ...
        if username=='Username' or number=='Phonenumber':
            messagebox.showerror('Invalid',"Type valid username and phonenumber")
    ...
    phonenumber = Entry(frame,width=45,fg='black',border=0,bg='white',font=('Microsoft Yahei UI Light',11))
    phonenumber.place(x=30,y=150)
    phonenumber.insert(0, 'Phone Number')
    ...
    Button(frame,width=39,pady=7,text='Login',bg='orange',fg='white',border=0,command=signup).place(x=35,y=280)
...
def create():
    ...
    Entry(cr,width=30,textvariable=cname,font=('arial',12)).place(x=600,y=200) #candidate name
